...
contract('TRHToken', function (accounts) {
    var tokenInstance;
    it('initializes the contract with correct value', function () {
        return TRHToken.deployed().then(function (instance) {
            ...
            assert.equal(name, 'TRH Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function (symbol) {
            assert.equal(symbol, 'TRH', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function (standard) {
            assert.equal(standard, 'TRH Token v1.0', 'has the correct standard');
        })
    });
